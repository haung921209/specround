...
지금 `ext` 를 실제로 쓰는 것은 **앵커를 base 에서 직접 자르지 않은 두 입력 표면**이다.
둘 다 남기는 사실은 같다 — "이 앵커는 base 가 아닌 텍스트에서 잘려 사다리로 옮겨왔고,
어느 단으로 왔다". base 에서 직접 고른 코멘트와 구별이 안 되면 §4 가 `fuzzy` 를 두는
이유가 comment.add 축에서 사라진다. `position` 이면 옮겨진 게 없으니 안 남긴다.
