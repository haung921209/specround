...
지금 `ext` 를 실제로 쓰는 것은 둘이다. 하나는 웹뷰다. diff 모드에서 **개정본 쪽 스팬**을 골라 단
코멘트는 재앵커 사다리를 거꾸로 돌려 base 앵커를 얻는데, 그때 어느 단으로 왔는지를
`ext.view = {"space": "revision", "strategy": …, "ambiguous": …}` 로 남긴다(`position`
이면 옮겨진 게 없으니 안 남긴다). base 에서 직접 고른 코멘트와 구별이 안 되면 §4 가
`fuzzy` 를 두는 이유가 comment.add 축에서 사라지기 때문이다.

다른 하나는 **외부 코멘트 흡수**(H9)다. 다른 도구에서 만들어진 코멘트를 들여올 때
...
둘 다 승격 후보이고, 아직 후보다 — 승격은 major 를 올리는 일이다.
