...
| G5 | **서버 0 · git 비의존** — 원장·스냅숏은 문서 곁 평문 append-only 파일. git 은 공유·내구의 **선택층**(원장을 커밋하면 clone = 전체 이력)이지 동작 전제가 아니다 |
...
- **resolve / reopen = 원장 이벤트** (사용자 결정 2026-08-06). 스레드 종결은 처분(G3)과
  다른 축이다 — 처분은 코멘트 하나의 결과(반영/기각/답변/보류)고, resolve 는 "이 대화는
  끝났다" 는 스레드 상태다. 누가 닫았는지(사람/에이전트)를 이벤트에 기록하고, 잘못 닫았으면
  reopen 을 append 한다(append-only 라 자연스럽다). 모든 뷰(웹뷰 3모드·CLI 목록)는 resolved
  를 기본 숨김 + 토글로 보인다.
